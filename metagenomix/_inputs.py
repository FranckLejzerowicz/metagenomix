...
def get_contigs_from_path(
    ...
    pool : str
        Name of a whole co-assembly pool to return all groups' contigs

    Returns
    -------
    contigs
        Path(s) to the contigs file
    """
    adx, assembler = get_assembler(self)
    assembler_hash = self.hashes[tuple(self.path[:(adx + 1)])]
    assembly = self.softs[assembler][assembler_hash].outputs
    if pool:
        contigs = {x: y[0] for x, y in assembly[pool].items() if x[0] == tech}
    else:
        assembly = assembly[self.sam_pool]
        if ret_list:
